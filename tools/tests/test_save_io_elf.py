...
for width in (8,16,32):
    for initial_state in (0,1,2,3,4):
        cpu=machine(sections)
        cpu.mem_write(symbols['gSaveData'],b'\0'*4)
        cpu.mem_write(symbols['gGbaSaveShared'],bytes([initial_state]))
        cpu.reg_write(UC_ARM_REG_CPSR,0xd1)
        cpu.reg_write(UC_ARM_REG_SP,0x0300f000)
        cpu.reg_write(UC_ARM_REG_LR,0x07000000)
        cpu.reg_write(UC_ARM_REG_R8,0x0e000000)
        cpu.reg_write(UC_ARM_REG_R9,0x79)
        def skip_drain(c,pc,size,_):
            if size==4:
                instruction=struct.unpack('<I',c.mem_read(pc,4))[0]
                if instruction&0x0f000010==0x0e000010 and (instruction>>8)&15==15:
                    c.reg_write(UC_ARM_REG_PC,pc+4)
        cpu.hook_add(UC_HOOK_CODE,skip_drain)
        cpu.emu_start(symbols[f'memu_store{width}Sram'],0x07000000,count=1000)
        assert cpu.reg_read(UC_ARM_REG_PC)==0x07000000
        assert bytes(cpu.mem_read(symbols['gSaveData'],width//8))==bytes([0x79])*(width//8)
        assert cpu.mem_read(symbols['gGbaSaveShared'],1)==bytes([4 if initial_state==4 else 1]),(width,initial_state)
        passed(f'guest-sram-store-{width}-state-{initial_state}')
